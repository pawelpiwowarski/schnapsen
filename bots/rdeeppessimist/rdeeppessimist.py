...
class Bot:
	...
	def make_pessimistic_assumption(state: State):
		...
		oldPerspective = state.get_perspective()
		
		## we find the current unknowns.
		unknowns = [index for index, card in enumerate(oldPerspective) if card == "U"]
		...
		other_player_unknowns = 5 - oldPerspective.count(other_player_term)
		...
		toOtherPlayerHand, toStock = splitcards(unknowns, other_player_unknowns, numberOfUnknownForStock)

		# we nnow create a new perspective, thereby also filling in the unknowns
		newPerspective = list(oldPerspective)
		for i in toOtherPlayerHand:
			newPerspective[i] = other_player_term

		trump_index = oldPerspective.index("S")
		...
		assert newPerspective[trump_index] == 'S' #Better safe than sorry

		for i in toStock:
			newPerspective[i] = "S"
		#the whole state should be known by now		
		assert not 'U' in newPerspective
		...
		current_player_perspective = oldPerspective # the current player hasn't obtained more knowledge compared to what was known before
		other_player_perspective = list(newPerspective) #we make a copy of the new state of the cards and will then blind out things
		...
		other_player_perspective = ['U' if cardStatus == current_player_term for cardStatus in other_player_perspective]
		# If a card is marked as being on the stock, the other player cannot now, except for the trump card, which we set back
		other_player_perspective = ['U' if cardStatus == 'S' for cardStatus in other_player_perspective]
		...
		deck = Deck(newPerspective, stock, player1_perspective , player2_perspective, trump_suit)
